...
def generate_platforms(grid_width, grid_height, cell_size, platform_width, platform_height, num_platforms, y_offset=0):
    ...
    while y < grid_height + y_offset and len(platforms) < num_platforms:
        ...
        # Place normal, breakable, spike, or orange platforms, avoiding rows with moving platforms
        for x in row_x_positions:
            ...
            if random.random() < 0.5 and y not in rows_with_moving_platforms:  # 50% chance to place a platform
                ...
                elif random.random() < 0.05:  # 5% chance to place an orange platform
...
class OrangePlatform:
    def __init__(self, rect):
        self.rect = rect
        self.color = (255, 165, 0)  # Orange color for platforms that double jump power
...
gravity = 0.7
jump_strength = 15.0
base_jump_strength = jump_strength  # Base jump strength for resetting
    ...
    for platform in platforms:
        ...
        if player_rect.colliderect(platform.rect):
            ...
            if isinstance(platform, OrangePlatform):
                jump_strength = base_jump_strength * 2  # Double jump power for orange platform

            if not isinstance(platform, OrangePlatform):
                jump_strength = base_jump_strength  # Reset jump power if hitting any non-orange platform

    if not hit_platform:
        jump_strength = base_jump_strength  # Reset jump power if no platform was hit
